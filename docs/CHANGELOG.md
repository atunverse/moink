...
> **★ 当前基线版本：fw R1.0.6 + page R1.0.10**（2026-09-17 用户确认功能基本正常，已固化至
> `moink/releases/baseline-fwR1.0.6_pageR1.0.10/`：整包/OTA 固件 + 页面 + 源码快照）。
> 后续版本如需回退，以此目录为恢复点。

> **维护规则**（2026-09-17 起，用户要求）：
...
- page：**高级滑杆百分比显示错误（旧 bug）**——显示公式把滑杆值（0~100）又除了 sc(100)，
  ...
  本次格式化重构时暴露。
- page：**高级滑杆数值改为纯数值**——按用户要求去掉 °/%/× 单位后缀，统一数值表示
  （亮度/对比度保留正负号标识方向）。
...
### 新增
- page：**竖屏编辑空间（用户主要使用竖屏模式）**——编辑/预览空间改为 552x768 竖屏
...
- page：**文字底色默认改回透明**——按用户要求撤销 R1.0.13 的白底默认：`newObj` 默认
...
## [fw R1.0.6 + page R1.0.10] 2026-09-17 ★基线版本（用户确认，已固化至 releases/）

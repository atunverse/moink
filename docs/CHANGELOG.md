...
> **★ 当前基线版本：fw R1.1.1 + page R1.1.1**（2026-09-23，A1 档位文案精简 + 离线预览可见 + OTA 同步页面；api 保持 2）。
> 上一基线 fw R1.1.0 + page R1.1.0（tag `fw-R1.1.0_page-R1.1.0`，内容 = 本版去掉 FB-011/FB-012，发布后即被本版替换；差分回退见 `backups/R1.1.0_R1.1.0_pre-R1.1.1/`）。
> 历史基线 fw R1.0.11 + page R1.0.26（tag `fw-R1.0.11_page-R1.0.26`）、fw R1.0.8 + page R1.0.22（tag `fw-R1.0.8_page-R1.0.22`）与 fw R1.0.6 + page R1.0.10（`moink/releases/baseline-fwR1.0.6_pageR1.0.10/`）保留作恢复点。
...
## [fw R1.1.1 + page R1.1.1] 2026-09-23

### 新增
- **`/api/ota` 可选查询参数 `?sync_page=1`**（FB-012）：固件校验通过、确定切槽重启后，一并清除设备上的页面热更标记（NVS `web_len` / `web_crc` / `page_ver`），重启后 `GET /` 直接返回新固件的内嵌页面。解决「刷了固件，但 web 分区里的旧页面永远优先、界面看不出变化 → 误以为固件没生效」的隐形陷阱。控制页「系统维护与升级 → ① 固件」下新增 **「同步更新页面」复选框**，勾选后自动带上该参数（**默认不勾**）。
- **离线预览能力补全**（FB-011）：本地（未连设备）打开页面时，A1「驱动档位」下拉与 800×600 编辑空间照常显示，A1 档位的界面回归可在离线状态完成。

### 修改
- **A1 驱动档位下拉文案精简**：四档统一为「方案D 相位1 / 方案D 相位2 / 方案A 原生800×600 / 方案A′ 原生800×600」，去掉「默认 · 与卖家固件同映射 / 相位对照 / 诊断」等后缀；同时移除该行下方的整段原理说明（`a1Hint` 元素及其 JS 引用一并删除）。
- 页面 `DEV_API` 由二值改 **三态**（`0 = 未连设备 / 能力未知`、`1 = 旧固件`、`2 = R1.1.0 及以后`），新增 `apiAtLeast2()`（`DEV_API === 0 || DEV_API >= 2`）；`syncPanelRows()` 与 `panelGeomOf()` 统一改用它。未连设备按「可用」乐观显示，连上设备后一律以真实 `api` 严格收紧（`api = 1` 仍隐藏下拉，`savePanel` 也只在**确知** `api = 1` 时提示需升级固件）。

### 修复
- **FB-011**：离线预览时 A1「驱动档位」下拉与 800×600 编辑空间不显示（原 `DEV_API` 初值 1，把「未连设备（能力未知）」与「旧固件（确实不支持）」合并成了同一态）。
- **FB-012**：固件升级不更新控制页，必须事后手动热更页面（现可用 `?sync_page=1` 一步完成）。

### 涉及文件
- `src/ota_web.c` —— `ota_handler` 读 `sync_page` 查询参数并调用 `ota_web_clear()`（**复用既有函数，未新增接口**）
- `src/ota_web.h` —— 路由注释补 `?sync_page=1` 说明
- `src/version.h` —— fw / page → R1.1.1；**api 保持 2**
- `page/index.html` —— 档位文案精简 + `a1Hint` 移除 + `DEV_API` 三态 + `apiAtLeast2()` + OTA「同步更新页面」复选框；版本 meta → R1.1.1
- `tools/smoke_page.js` —— 新增 6 条 R1.1.1 断言（含 `DEV_API` 0/1/2 三态行为、文案精简、`sync_page` 拼接、`a1Hint` 移除）

### 影响范围与注意事项
- **api = 2 不变**：帧格式、色码、几何、算法全部未动；`?sync_page=1` 是**可选**查询参数，不带则 `/api/ota` 行为与 R1.1.0 完全一致 → 旧页面 / 旧客户端零影响，无需迁移。
- **必须刷固件**（固件侧行为有新增）→ 按 `使用说明.txt` 走 FlashKit 或 OTA。**刷的时候建议勾选「同步更新页面」**，可一次把控制页也切到 R1.1.1，免去事后热更。
- 清除热更标记**只删 NVS 三个键，不擦 web 分区数据**；若想保留自己热更的页面，不勾选该复选框即可（行为与 R1.1.0 一致）。
- 回退：`backups/R1.1.0_R1.1.0_pre-R1.1.1/`（`page-index.html` / `ota_web.c` / `ota_web.h` / `version.h`），或刷回 fw R1.1.0 + page R1.1.0。
- 门禁：编译 RAM 12.2% / Flash 61.4%；`check_frame.py` PASS；`smoke_page.js` ALL PASS（含 6 条 R1.1.1 新断言）；`_r110_regress.js` ALL PASS（768×552 路径与 R1.0.26 逐字节一致）；`check_algo.py` 基线 B2 12/12 MATCH。

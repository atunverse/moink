...
> **★ 当前基线版本（已发布）= R1.2.0**（tag `R1.2.0`，2026-09-24，**替换**旧 tag `fw-R1.1.1_page-R1.1.1`；版本号合并 + 统一升级入口 + 强制重置 + 清除过时代码 + 分区表新增 store 存储分区；api 保持 2）。
> **★ R1.2.0 起 tag 命名随版本号合并简化为单一 `R1.x.y`**（不再用 `fw-R1.x.y_page-R1.x.y` 双号）。
> **⚠️ 升级到 R1.2.0 必须线刷，不能 OTA**：本版分区表有变（新增 store 存储分区，FB-017 第一部分），
> 分区表只能整包线刷改写 —— 从 R1.2.0 起（同分区表）恢复正常 OTA。
> **R1.2.0 起不再有 fw / page 两套版本号**：固件与控制页共用 `MOINK_VERSION`，运行时 `/api/info` 的 `ver` = **当前生效的控制页版本**，`fw` 仅供前端判断热更页是否落后。
> R1.1.3 那个验证包（A1 驱动收敛为「768×552 顺序直写」+ 设置就地保存状态）**未经上机**，已整体并入 R1.2.0 —— 同版本号只保留一份产物，避免同名不同内容。
...
> **维护规则**（2026-09-17 起）：
> 1. 任何固件或控制页改动合入时，**必须在本文档新增条目**，条目格式保持统一；
> 2. 版本号（R1.2.0 起）：固件与控制页**共用单一版本号** `MOINK_VERSION`（控制页内表现为
>    `<meta name="moink-page-version">`，两处由 `make_index_header.py` 断言一致）。
>    `api`（帧格式契约号）与发布版本号**解耦**，只在帧契约变化时 +1；api 变化属重大事件，
>    必须在「影响范围」标注；
> 3. 条目按时间**最新在上**排列；
...
## [R1.2.0] 2026-09-24 —— 版本号合并 + 升级入口合并 + 升级强制重置 + 过时代码清除（FB-015）

> **已发布（tag `R1.2.0`，`--latest`，2026-09-24 发布，替换并删除旧 tag `fw-R1.1.1_page-R1.1.1`）。**
> R1.1.3（A1 驱动收敛 + 设置就地保存状态）未经上机，
> 内容已整体并入本版；验收按文末「真机验收清单」执行。
> **★★ 本版分区表有变（新增 store 存储分区，FB-017 第一部分）：升级到本版必须用 FlashKit
> 线刷（merged.bin 含新分区表），不能从旧版本 OTA。**

### 新增
- **store 原始帧存储底座（FB-017 第一部分）**：分区表新增 `store` 分区（0x2F0000 起，
  1,088 KiB），按 **8 个 128 KB 槽位**规划（单槽容纳最大帧 800×600 2bpp = 120,000 B）；
  本版只做分区探测与容量上报（`src/store.c`，`/api/info` 新增 `store_slots` 字段），
  轮播的上传 / 调度 / 播放在第二部分接入。分区表只能靠整包线刷改写 —— 这正是把存储底座
  提前并入本版的原因：让用户只线刷一次。写入策略按「整槽轮转 + 游标存 NVS」设计，
  延寿依据见 `docs/调研-内置Flash延寿方案.md`。
- **统一升级入口 `POST /api/upload`**：固件与控制页走同一入口，用户只上传一个文件、**不需要判断**
  这次升的是哪一种。固件按请求体**首块内容**嗅探分流 —— 首字节 `0xE9` = 固件；前 1 KB 内出现
  `<!doctype` / `<html`（按 ASCII 小写匹配）= 控制页；其它一律 `400` 且**判定前不擦写任何分区**。
  **不用** `Content-Type`（控制页为规避跨源预检，一律包成 `text/plain` 的 Blob）**也不用**文件名
  （Blob 上传没有 `filename`）。固件 `sniff_kind()` 与页面 `sniffKind()` 是同一套规则，两处源码里
  互相注明「必须同步修改」。
- **单一版本号 `MOINK_VERSION`**：控制页页脚与 `<meta name="moink-page-version">` 与之同源；固化前由
  `tools/make_index_header.py` 断言两处一致，不一致直接拒绝生成内嵌页头（**防版本漂移门禁**，
  已用反向测试验证：把页面 meta 改成 R1.1.3 后生成被拒绝且打印两边实际值）。
- **固件升级强制重置**：`settings_reset_for_upgrade()` —— OTA 校验通过、切槽之后擦除 `panel` /
  `hflip` / `a1_mode` / `wifi_pwr` / `sleep_s` / `wake_s` 六键并回落默认，**保留 `ap_ssid` / `ap_pass`**
  （避免升级后热点改名导致再也连不上）。
- **热更控制页落后于固件时的红字警示**：`/api/info` 保留 `fw` 字段仅供前端比较；运行时 `ver`
  （当前生效版本）低于 `fw` 时状态栏版本号转红，并提示重新上传固件。

### 修改
- **分区表重排（R1.2.0 起生效）**：`ota_0` / `ota_1` 1.5 MB → **1.25 MB**（R1.2.0 app 968 KB，
  槽位余量约 35%）；`web` 960 KB → **384 KB**（页面 143 KB 的 2.7 倍）；余量给
  `store` **1,088 KiB**（1.0625 MiB，8 × 128 KB 槽位 + 64 KiB 预留）。`nvs` / `otadata` / `phy` 不变。
- **`/api/info`**：原 `fw` / `page` 两字段合并为 **`ver`**（当前生效的控制页版本：热更页在用时取其
  meta 版本，否则 = `MOINK_VERSION`）；`fw` 保留一版用于上述落后判定；新增 `store_slots`
  （存储槽位数，0 = 分区缺失）；`api` 不变（仍为 2）。
- **热点设置改单字段语义（FB-014①）**：`settings_set_ap()` 的 `ssid` / `pass` 任一为 NULL 表示
  「保持当前值」，`pass` 空串表示清除密码（热点转开放）；页面「留空 = 保持当前密码」，
  新增「清除密码」勾选（仅已设密码时显示），两者与旧行为兼容。
- **设置保存先应答后应用热点（FB-014②）**：`settings_post_handler` 先把 `200` 应答发回页面，
  300 ms 后再经 esp_timer 一次性回调应用 AP 配置 —— 改名 / 改密码踢掉手机连接时，页面
  已经拿到成功应答，不再依赖「连接被切断后靠回读猜」。
- **控制页状态栏**：`固件 / 页面 / 接口` 三行改为 `版本（`i-verD`）/ 接口（`i-apiD`）`两行。
- **`POST /api/frame` 几何接受规则**：A0 只接受 768×552（原为 A0 / A1.1 各只收自身画像尺寸）。
- **控制页响应加 `Cache-Control: no-store`**：内嵌页与控制页热更响应都加，避免页面随固件更新后
  仍命中浏览器缓存、被误判成「升级没生效」。
- **`tools/make_index_header.py`**：新增版本一致性断言与失败信息。
- **`tools/smoke_page.js` / `tools/_r110_regress.js`**：断言随 R1.2.0 更新（见「影响范围」）。

### 修复
- **FB-014① · 热点密码单字段更新**：原实现要求 `ssid` / `pass` 成对出现，页面总是把空密码一起发
  → 「想改热点名却意外清掉了密码（热点转开放）」。现固件支持单字段更新（缺省 = 保持），
  页面留空不再发送 `pass`，清除密码改为显式勾选「清除密码（改为开放热点）」。
- **FB-014② · 保存先应答后应用热点**：原实现 `settings_post_handler` 先 `netif_ap_apply()`（踢客户端）
  再发应答，改名场景页面收不到 200。现改为先应答、300 ms 后经 esp_timer 延迟应用热点配置。
- **FB-016 · std 标准档纯红杂点**：`page/index.html` 里 std 档红目标色由 `(220,0,0)` 改为 `(230,0,0)`
  （`STD_LUT[0][220]` → `STD_LUT[0][230]`）。实测 220 在屏上出现杂点，调整为 230 以消除杂点（230 为无杂点最低值）。
  仅改 std 档红调色板索引，**不影响算法冻结**（`check_algo.py` 的 `STD_MARK` 不含该索引，
  `quantize` 剥离 std 块后仍与 M7 逐字一致）；`check_algo.py` / `smoke_page.js` 复跑均 PASS。
  （该值原为 FB-002 Stage A 真机定稿值，改动后建议重做一轮纯红样张比对核验。）

### 涉及文件
- 分区表：`partitions.csv`（R1.2.0 起新布局，必须随 merged.bin 整包线刷）
- 固件：`src/version.h`、`src/main.c`、`src/ota_web.c`、`src/ota_web.h`、`src/settings.c`、
  `src/settings.h`、`src/epd_drv.c`、`src/epd_drv.h`、`src/store.c`、`src/store.h`、
  `src/CMakeLists.txt`、`src/index_html.h`
- 页面：`page/index.html`
- 工具：`tools/make_index_header.py`、`tools/smoke_page.js`、`tools/_r110_regress.js`、
  `tools/_r120_patch_drv.py`、`tools/_r120_patch_main.py`、`tools/_r120_patch_page.py`、
  `tools/_r120_patch_gates.py`、`tools/_r120_patch_docs.py`

### 影响范围与注意事项
- **★★ 分区表已变更：升级到本版必须线刷，不能 OTA（FB-017 第一部分）**。R1.1.x 设备
  OTA 本包 `moink_R1.2.0_app.bin` 后可以开机（旧分区表缺失 store 时固件会告警并报
  `store_slots=0`，其余功能不受影响），但**没有轮播存储分区**，且这种升级方式不受支持 ——
  请一律用 FlashKit 刷 `moink_R1.2.0_merged.bin`（含新分区表）。自 R1.2.0 起（同分区表）
  恢复正常 OTA：R1.2.0 → 之后版本可在网页里升级。
- **删除的路由（不向后兼容）**：`POST /api/ota`（含 `?sync_page=1`）与 `POST /api/web` 已整体删除，
  统一改用 `POST /api/upload`。R1.1.x 的旧控制页无法再对本版设备发起升级（收到 `404`），
  页面会提示「设备固件过旧，不含统一升级入口」—— 请先用 R1.2.0 的控制页。
- **删除的过时代码**：A1.1 诊断变体（`EPD_PANEL_A11`、`epd_set_a11_variant()` 与 V1~V8 路径、
  `a11_var` 设置字段与控制页 UI）、A1 交错相位档（旧值 3 / 4）、`MOINK_EMBED_PAGE` 逃生开关与
  `FALLBACK_HTML` 占位页、死代码 `epd_write_frame_v78()` / `epd_write_frame_linear()`、
  `spi_reconfigure_clock()` 与 `A11_V6_SPI_HZ` 降速分支。
- **升级会重置设置**（热点名与密码除外）：`panel` / `hflip` / `a1_mode` / `wifi_pwr` / `sleep_s` /
  `wake_s` 全部回默认。**A1 屏默认档是「768×552 顺序直写」，不需要手动设白边** ——
  页面侧画面参数（风格 / 强度 / 文字 / 旋转 / 方向）仍存在手机 `localStorage`，不受升级影响。
- **api 保持 2**：帧格式、色码、几何、引脚契约均未变（`check_frame.py` PASS、`check_algo.py` 12/12 MATCH）。
- **升级后请人工确认一次 A1 档位下拉是「768×552 顺序」**（`a1_mode` 语义在 R1.1.1→R1.1.3 之间被
  重编号，虽然本版会强制重置为默认值 1，仍建议肉眼确认）。
- 编译体积（0 告警）：RAM **39,492 B（12.1%）**、Flash **968,840 B（新 1.25 MB ota 槽的 73.9%，
  槽位余量约 26%）**；较 R1.1.1 净变化 RAM −608 B / app +3,870 B（store 底座 + FB-014 计时器 +
  控制页增量，扣除驱动与路由删减）。
- 回退：`backups/R1.1.3_pre-R1.2.0/`（含 `src` / `page` / `tools` / `docs` 快照与 `_baseline.txt`）；
  降级包保留 `moink_R1.1.1_app.bin`（首选）与 `moink_R1.1.3_app.bin`。

### 真机验收清单
> **前置：R1.1.x → R1.2.0 必须线刷**（分区表变更）。先按 FlashKit 刷 `merged.bin`，
> 再执行以下各项（V1 / V2 用同版本包重复上传来验证统一入口）。
1. **V1 统一入口·固件**：线刷后，设置页上传 `moink_R1.2.0_app.bin` → 设备重启；`/api/info` 的 `ver` 与
   `fw` 都是 `R1.2.0`；设置项回到默认，**热点名与密码保持不变**。
2. **V2 统一入口·控制页**：上传 `page-R1.2.0.html` → **不重启**，状态栏版本变为热更页版本；
   再上传一个 R1.1.x 旧页（版本号更低）时，状态栏版本号应**转红**并提示重新上传固件。
3. **V3 A1 顺序直写**（并验收 FB-013）：A1 屏默认档出图**满屏无白边、方向正向、无细密条纹**；
   切换「方案A 原生800×600」应看到右 32 列 / 下 48 行被裁（作为对照档预期行为）。
4. **V4 强制重置生效**：升级前把 A1 档位切到「方案A 原生800×600」、改动休眠档；升级后应回到
   「768×552 顺序」与默认休眠档，而热点名 / 密码仍为升级前的值。
5. **V5 存储分区就位（FB-017 第一部分）**：串口日志出现 `store: off=0x2f0000 size=1088 KB,
   8 slot(s) x 128 KB`；`/api/info` 返回 `"store_slots":8`。
6. **V6 热点单字段语义（FB-014①②）**：已设密码的设备上留空密码、只改热点名保存 →
   密码仍在（重连需密码）；勾选「清除密码」保存 → 热点转开放；改名保存时页面应直接
   出现「✓ 已保存并生效」（不再依赖回读兜底）。


---

## [fw R1.1.3 + page R1.1.3] 2026-09-23 —— ⚠️ 验证版（未上机，已整体并入 R1.2.0）

### 新增
- **设置项「就地保存状态」**（控制页）：休眠 / 热点 / 屏幕三处保存按钮后各有一个状态胶囊，
  显示 `保存中…` / `✓ 已保存并生效` / `✗ 未生效：<项名>` / `✗ 保存失败：…` / `⚠ 没收到应答…`。
  根因：全局提示 `#msg` 挂在「图片」标签页里，切到「设备」标签页保存时**写进去的提示不可见**，
  连保存失败都是静默的（社区反馈：「热点设置保存之后没有生效，刷新一下还是原来的……可以加个保存成功的提示」）。
- **保存后回读核对**：POST 之后回读 `/api/settings` **逐字段比对**，只有设备确认落库才报「已生效」——
  不再轻信 `HTTP 200`；读不到时自动重试 3 次 × 2.5 s（覆盖「热点刚切换、手机还没重连上」的窗口）。

### 修改
- 「同步更新页面」复选框改为**默认勾选**（刷固件即顺手把控制页切回内嵌新页面）。
- 热点密码输入框提示由「留空则不修改」改为「**留空 = 清除密码，改为开放**」——与固件真实行为一致
  （`netif_ap_apply()`：`ap_pass` 为空即 `WIFI_AUTH_OPEN`）。原提示与行为相反，属误导。
- 保存成功提示 8 秒后自动淡出；**失败与「待核对」常驻**，直到下一次操作才清除。

### 修复
- 设备页保存设置「无任何反馈」（含保存失败静默）——见「新增」第 1 条。
- `savePanel` 在 `api = 1` 的旧固件上不再谎报「已保存并生效」，改为「✓ 已保存（当前固件 api=1 不支持 A1 档位…）」。

### 涉及文件
- `page/index.html`：新增 `.savestate` 四态样式（busy / ok / warn / err）与
  `saveState / readSettings / verifyFields / postSettings`；三处就地状态位；
  `fwSyncPage` 默认勾选；密码提示语；版本 meta → R1.1.3。
- `src/version.h`：fw / page → R1.1.3（**api 保持 2**）；`src/index_html.h` 重生成（内嵌页 142,208 B）。
- 门禁：`tools/smoke_page.js`（+8 条 R1.1.3 断言）、`tools/_r110_regress.js`（默认新页改指 `page-R1.1.3.html`）。

### 影响范围与注意事项
- **api 保持 2**：未动任何接口与帧格式，未跟进热更的旧页面帧仍被接受。
- 固件本体仅随内嵌页面更新：Flash 61.3% → **61.6%**（+4,150 B，全是页面增量）；RAM 12.2% 不变。
- **R1.1.2 验证包未经上机即并入本版**：本版固件同时含「A1 驱动 768×552 顺序直写」与页面改进，
  刷一次即可完成两项验证；本地不再保留同名 R1.1.2 产物（避免同名不同内容）。
- 门禁：`check_frame.py` PASS、`smoke_page.js` ALL PASS、`_r110_regress.js` ALL PASS
  （768×552 路径与 R1.0.26 逐字节一致）、`check_algo.py` MATCH（基线 B2）；
  固件内嵌页与 `page/index.html` **逐字节一致**。
- 回退：`backups/R1.1.2_pre-R1.1.3/`（page + version.h）、`backups/_docs_pre_r113/`（两份文档）。

## [fw R1.1.2 + page R1.1.2] 2026-09-23 —— ⚠️ 验证版（本地验证包，未发 Release）

### 新增
- 无（本版为 FB-013 驱动收敛，未新增接口与开关；api 保持 2）

### 修改
- ★ **A1 驱动改为「768×552 顺序直写」**（`a1_mode 1`，**默认**）：帧几何 = 玻璃可见区 768×552，
  TRES / 刷新窗 768×600，写入窗 `(0..767, 0..551)` + 单次 `0x10` 连发 552 行 × 192 B，
  栅极号 = 行号（顺序 0..551）。目标是 1:1 铺满可见区：**不裁切、不重采样、无白边**（与 A0 观感一致）。
  行内**固定整行镜像**（真机对照确认这块屏只有镜像后才正向），不读「左右镜像」开关。
- **A1 驱动档位由四档收敛为两档**：`1 = 768×552 顺序（默认·正解）` / `2 = 方案A 原生800×600（对照）`。
  页面下拉同步精简为两项，`A1MODE` 默认 1、`A1MODE_MAX` 4 → 2，`panelGeomOf()` 只在档 2 且 api ≥ 2 时给 800×600。
- **删除已被真机否证的交错路径**：`a1_ilv_y()`（`2i / 2i−599` 映射）、`epd_write_frame_a1_ilv()`（逐行单行窗 + 0x10）
  整块移除；新增 `epd_write_frame_a1_seq552()` —— 复用方案A 那条已被真机验证可用的写入路径（整窗 + 单次 0x10 + 整帧 CS 包络），只换几何。
- NVS 里遗留的旧档位值 `3` / `4` 读入时**统归一到档 1**（`settings.c` 与 `epd_set_a1_mode()` 双重钳制）。
- `row_a1()` 的适用范围收窄为「2 NATIVE800 对照档」；`epd_set_hflip()` 注释同步说明生效范围。

### 修复
- **FB-013**：A1 屏只有「方案A 原生800×600」档可驱动，且画面右 32 列 / 下 48 行被裁切（可见区恒 768×552）。

### 涉及文件
- `src/epd_drv.h` —— A1 两档模式定义与说明（旧四档宏删除）
- `src/epd_drv.c` —— `epd_write_frame_a1_seq552()` 新增；`a1_ilv_y()` / `epd_write_frame_a1_ilv()` 删除；
  `profile_apply()` / `prepare_geom()` / 分发逻辑 / `epd_clear_cycles()` 同步更新
- `src/settings.c` —— `a1_mode` 钳制范围 1..2
- `src/version.h` —— 版本号与变更说明
- `page/index.html` —— 下拉两项 + 几何判定 + 版本 meta

### 影响范围与注意事项
- **api 保持 2**：帧几何仍为 768×552 / 800×600 两种，帧头字段与偏移不变；页面对 800×600 帧仍发版本 2。
  未跟进热更的旧页面发来的 768×552 帧**仍能正确显示**（落到默认档路径）。
- **A0 / A1.1 路径零改动**（既无函数改动也无行为改动；`_r110_regress.js` 的 768×552 路径与 R1.0.26 逐字节一致）。
- ⚠️ 本版是**待真机确认的验证版**：正式 Release 待默认档在真机上确认「满屏无裁切 + 方向正确」后再发布。
- 回退：`backups/R1.1.1_R1.1.1_pre-R1.1.2/`（version.h / epd_drv.h / epd_drv.c / settings.c / page-index.html）。
